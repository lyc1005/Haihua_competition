...
class RCProcessor(DataProcessor):
    def __init__(self):
        ...
        for sid in range(3):
            data = []
            with open(["train_1.json", "dev_1.json", "test_1.json"][sid], "r", encoding="utf-8") as f:
                data += json.load(f)
            if sid == 0:
                random.shuffle(data)
...
def main():
    ...
    if args.do_eval:
        ...
        opt_mapper = {0:'A',1:'B',2:'C',3:'D'}
        test_pred_opt = [opt_mapper[i] for i in test_pred]
        assert len(test_idx)==len(test_pred_opt)
        test_results = np.array([test_idx, test_pred_opt]).T.tolist()
        with open(os.path.join(args.output_dir,'test_output.csv'), 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerows([['id','label']])
